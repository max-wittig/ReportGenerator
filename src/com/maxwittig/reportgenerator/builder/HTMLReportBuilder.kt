...
class HTMLReportBuilder(timekeeperTasks: MutableList<TimekeeperTask>, private val reportType: ReportType,
                        todaysDate: Date = Date()) : ReportBuilder(timekeeperTasks, reportType, todaysDate = todaysDate) {
    private val stringWriter = StringWriter()
    private val html = Html(stringWriter)
    ...
    private fun addHead() {
        val head = html.head()
        val css = FileUtils.getFileContentFromJar("/css/reportStyle.css")
     ...
     */
    private fun addHTML(tasks: MutableList<TimekeeperTask>, body: Html, currentReportType: ReportType,
        ...
        addProjectTable(tasks, body, createProjectHolder(tasks))
    }

    private fun addProjectTable(tasks: MutableList<TimekeeperTask>, htmlElement: Html, projectHolder: ProjectHolder) {
    ...
    private fun addTaskTable(tasks: MutableList<TimekeeperTask>, htmlElement: Html, format: SimpleDateFormat) {
        val table = htmlElement.table()
